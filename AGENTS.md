...
On every push and PR, `lint` builds `checks.x86_64-linux.pre-commit-check` and `darwin-eval` evaluates `kebee` on GitHub-hosted runners. `build` builds the three Linux hosts sequentially and pushes their explicit outputs to Cachix; it runs on the self-hosted runner container on `uribo-btw` for pushes to `main` and same-repo PRs, and on GitHub-hosted runners for fork PRs. The container auto-starts, its network isolation has been verified live, and it shares the host Nix daemon, so container resource limits do not constrain Nix builds.
