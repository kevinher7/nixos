...
On every push and PR, CI runs on GitHub-hosted runners: `lint` builds `checks.x86_64-linux.pre-commit-check`, `darwin-eval` evaluates `kebee`, and `build` builds the three Linux hosts sequentially before pushing their explicit outputs to Cachix. The self-hosted runner prototype on `uribo-btw` is disabled by default; its network isolation has been verified live. A maintainer may dispatch its guarded job only from `main` after enabling and manually starting that runner. It shares the host Nix daemon, so container resource limits do not constrain Nix builds.
